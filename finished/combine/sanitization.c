...
//function structure to dusplay all records in payment.dat
...
void payMethodMerchantAmount() {
	...
	char pay_method[20], pay_merchant[20], payAmountChar[10];
	bool validPayAmount = false;
	struct PaymentDetail p, pRecord[100];
	...
	printf("\t\tEnter the payment amount to search (RM)                                                      :");
	rewind(stdin);
	scanf("%s", &payAmountChar);
	//validation check for payment amount
	while (validPayAmount == false) {
		for (int i = 0; i < strlen(payAmountChar); i++) {
			if (isdigit(payAmountChar[i]) == 0) {
				validPayAmount = false;
				printf("\t\tInvalid payment amount!!\n");
				printf("\t\tPlease reenter the payment amount to search (RM)                                             :");
				rewind(stdin);
				scanf("%s", &payAmountChar);
				break;
			}
