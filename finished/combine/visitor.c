...
void visitorMenu() {
	...
	do {
		...
		case 2:printf("\t\t\tYou will now be directed to the Register page. Please press [ENTER] to proceed.\n");
			...
			break;
		case 3:printf("\t\t\tThank you for using our programme.\n\n");
...
void visitorLogin() {//==========================================
	...
	do {
		...
		printf("\n\n\t\t\t\t\t\t\tPlease enter your User ID No. [5 Characters]         > ");
		rewind(stdin);
		scanf("%[^\n]", &currentUserId);
	...
	do {
		...
		printf("\t\t\t\t\t\t\tPlease input password  (Type 'XXX' to MENU )         > ");
		rewind(stdin);
		scanf("%[^\n]", currentPassword);
		...
		if (passwordValidation == 1) {
			...
			printf("\t\t\t\t\t\t\tPassword does not match. Please try again. ( Press XXX to exit )\n\n");
...
void visitorRegisterAccount() {
	{
		...
		if (citizenshipStatus == 1) { // citizenshipStatus 1 = local 
			...
			/*strcpy(newAcc.visitorCountry, "MALAYSIA");*/
			/*newAcc.visitorPassportNo[10] = "---";*/

		//----REGISTER: Input new visitor ic number FOR LOCALS!!!!!!!!!
			do {
				printf("\nIdentity Card number (according to the Identity Card)         >");
				rewind(stdin);
				scanf("%[^\n]", &newAcc.visitorIcNo);
		...
		do {
			...
			printf("\n\n");
			printf("Contact Number    (e.g. 011 1111111)                        >");
			rewind(stdin);
		...
		do {
			printf("\n\nComfirmation to register for a new account (Y/N)    >");
			rewind(stdin);
		...
		if (createUserId == 1) {
			...
			fprintf(registerFptr, "\n%s|%s|%s|%s|%c|%02d/%02d/%02d|%s|%s|%s|1|", newAcc.visitorUserId, newAcc.visitorIcNo, newAcc.visitorPassportNo, newAcc.visitorName, newAcc.visitorGender, newAcc.visitorBd.day, newAcc.visitorBd.month, newAcc.visitorBd.year, newAcc.visitorEmail, newAcc.visitorContactNo, newAcc.visitorAddress);
...
void displayRecordFunction() {
	...
	//printf("|%5s|%12s|%6s|%25s|%7c|%04d/%02d/%02d|%20s|%12s|%65s|%6d|", delete[order].visitorUserId, delete[order].visitorIcNo, delete[order].visitorPassportNo, delete[order].visitorName, delete[order].visitorGender, delete[order].visitorBd.year, delete[order].visitorBd.month, delete[order].visitorBd.day, delete[order].visitorEmail, delete[order].visitorContactNo, delete[order].visitorAddress, delete[order].visitorAccStatus);
	//printf("-------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n")* / ;
	...
	do {
		...
		do {
			...
			case 1:// case 1: search with visitor user ID
				...
				do {
					printf("\nPlease enter your Visitor User ID                                     >");
					rewind(stdin);
					scanf("%[^\n]", &searchCmp.visitorUserId);
					/*searchCmp.visitorUserId = toupper(searchCmp.visitorUserId);*/
					strupr(searchCmp.visitorUserId);


					searchTempStruct = universalCharacterCheck(searchCmp.visitorUserId);

					if (searchTempStruct.noCharacter != 5) {
			...
			case 4:// search visitor name 
				...
				do {
					...
					printf("\n\n");
					printf("\nPlease enter the Visitor Full Name                                >");
					rewind(stdin);
			...
			case 5:// search visitor gender
				...
				do {
					...
					printf("\n\n");
					rewind(stdin);
					printf("\nPlease enter the Visitor Gender (M = MALE / F = FEMALE)           >");
			...
			case 7:// search visitor bd year
				...
				do {// validate date of birth (year)
					...
					/*printf("\nPlease enter the Visitor Date of Birth [month : 01-12]           >");*/
					rewind(stdin);
					scanf("%s", &tempYear);
			...
			case 9:// search contact no   ****************************
				...
				do {
					...
					printf("\nPlease enter the Visitor Contact Number                               >");
					rewind(stdin);
			...
			case 10:// search address m  
				...
				do {
					...
					printf("\n\n");
					printf("\nPlease enter the Visitor Home Address                                 >");
					/*printf("\nPlease enter the Visitor Contact Number                               >");*/
					rewind(stdin);
			...
			case 11:// search account status
				...
				do {
					...
					rewind(stdin);
					/*printf("\nPlease enter the Visitor Gender (M = MALE / F = FEMALE)           >");*/
					printf("Please enter the Visitor Account Status (1 = ACTIVE|0 = DELETED)      >");
			...
			default:
				printf("\nERROR. Please chose the from the available options.\n");
...
void deleteFunction() {
		...
		do {
			...
			do {
				// Validation to exclude (> or < 12 char/alphabnets/special characters)
				printf("\n\nPlease enter your Visitor ID No. to be deleted [5 Characters]         >");
				rewind(stdin);
				scanf("%[^\n]", &deleteUserId);
				printf("\n\n");
				...
				if (deleteTempStruct.noCharacter < 5 || deleteTempStruct.noCharacter> 5) {
				...
				else if (deleteTempStruct.noDigit > 4 && deleteTempStruct.noDigit <= 4)
				{
		...
		//DELETE PROCESSING 
		int existence = 0, deleteCondition = 0;
		if (existence = 1) {
			...
			do {
				...
				case 'y':
					...
					//closure
					printf("The termination the account of Visitor ID: %s has been processed. You will now be directedt to the Main Menu.\n", delete[order].visitorUserId);
					system("pause");
					system("cls"); loginHeader(4);
					return 0;
					break;
				...
				case 'N':
				case 'n':
					printf("The termination of your account has been cancelled. You will now be directedt to the Profile Menu.\n");
					system("pause");
					system("cls"); loginHeader(4);
					return 0;
...
void modifyFunction() {
	...
	do {
		...
		do {
			...
			case 2:// CASE 2: MODIFY contact no
				...
				do {
					...
					rewind(stdin);
					scanf("%[^\n]", &modify[order].visitorContactNo);
					/*strupr(modify[order].visitorCountry);*/
					/*	newAcc.visitorCountry[99] = strupr(newAcc.visitorCountry);
						printf("%s", newAcc.visitorCountry);*/
					...
					else if (modifyTempStruct.noDigit > 0)
					{
						printf("ERROR. You have entered %d digits(s). Please try again\n\n\n", modifyTempStruct.noDigit);
						modifyContactNo = 0;
					}
						...
						printf("Character check register country success. Proceed.\n");
						modifyContactNo = 1;
					}
				...
				for (int a = 0; a < i; a++) {
					...
					fprintf(modifyFptr, "%s|%s|%s|%s|%c|%02d/%02d/%02d|%c|%s|%s|%s|%d|", modify[a].visitorUserId, modify[a].visitorIcNo, modify[a].visitorPassportNo, modify[a].visitorName, modify[a].visitorGender, modify[a].visitorBd.day, modify[a].visitorBd.month, modify[a].visitorBd.year, modify[a].visitorEmail, modify[a].visitorContactNo, modify[a].visitorAddress, modify[a].visitorAccStatus);
		...
		} while (modifyChoice < 0 && modifyChoice >3);
...
void profileMenu(char userId[]) {
	...
	do {
		printf("\n\n\t\t\t\t\t\t       ---------------------------------------------------------\n");
		printf("\t\t\t\t\t\t       |        Dear, what would you wish to perform  ?        |\n");
		printf("\t\t\t\t\t\t       |                                                       |\n");
