...
void checkOut(char currentUserId[]) {
	...
	for (i = 0; i < num; i++) {
		if (strcmp(personalId, entry[i].in.id) == 0 && entry[i].out.outHour == 23 && entry[i].out.outMins == 59) {
			foundEntryId = 1;
...
void searchEntry(struct Entrance entry[50]) {
	...
	do {
		...
		//no matching record founded
		if (found == 0) {
			printf("\n  >> Sorry dear. No matching record exist \n");
			sound();
		}

		printf("\n   Do you wish to search another record?('Y'-continue search /'N'-Back to  VisitAndExit) -> ");
		rewind(stdin);
		scanf("%c", &ctn);
