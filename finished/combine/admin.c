...
void addAdmin() {
	...
	do {
		...
		if (strcmp("00000", admin.staff_id) == 0) { system("pause"); system("cls"); adminMenu(); }
	...
	//Get Password
	printf("\n\nPassword:"); scanf("%[^\n]", admin.password); rewind(stdin);
	printf("Re-enter Password:"); scanf("%[^\n]", pass); rewind(stdin);
...
void searchAdmin() {
	...
	do {
		...
		case 12:exit; system("pause"); system("cls"); adminMenu(); exit(1); break;
		default:
		...
		printf("\nContinue to Search ? (Y -Yes / N -No) >> "); scanf("%c", &yesNo); rewind(stdin); yesNo = toupper(yesNo);
	} while (yesNo == 'Y');
	system("pause"); system("cls"); adminMenu();
}
...
void modifyAdmin(int num) { //num = 1 = change personal detail else num = 2 = change password
	...
	//Reset for password
		...
		do {
			...
			scanf("%[^\n]", curPass); rewind(stdin);
			if (strcmp(curPass, savePass) == 0) {
				do {
					printf("\n\nEnter New Password:"); scanf("%[^\n]", admin.password); rewind(stdin);
					rewind(stdin); printf("Re-enter Password:"); scanf("%[^\n]", conPass); rewind(stdin);
...
void adminLogin() {
	int found = 0; char userID[6], password[30]; char yesNo;
	...
	do {
		...
		if (found == 1) {
			...
			system("pause"); system("cls"); adminLoginMenu(); break;
		}
		else if (found == 2) { system("cls"); loginHeader(1); printf("\t\t\t\t\t\t\t\t\t   User does not exits. Please Try Again:\n"); found = 0; }
		else { system("cls"); loginHeader(1); printf("\t\t\t\t\t\t\t\t\t   Incorrect Password !!! Please Try Again:\n"); }
	} while (found == 0);
	exit;
}
